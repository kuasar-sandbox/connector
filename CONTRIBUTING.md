...
Thank you for contributing to `kuasar-sandbox/connector`.
...
```bash
git clone git@github.com:<your-login>/connector.git
cd connector
git remote add upstream git@github.com:kuasar-sandbox/connector.git
```
...
Open a pull request to the appropriate target: `main` for current development, or a supported `release/vMAJOR.MINOR.x` maintenance branch. For example, use `<your-login>/connector:feat/short-description` → `kuasar-sandbox/connector:main`. Enable **Allow edits from maintainers** when appropriate.
...
[Apache License 2.0](LICENSE). The repository's non-Apache boundaries are
documented in [LICENSE_SCOPE.md](LICENSE_SCOPE.md).
