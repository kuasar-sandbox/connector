...
对外导出 `pkg/tapfd`(tapfd 交接协议 SDK,`sandbox-runtime` 作 consumer 直接
...
| 路径 | 角色 |
| --- | --- |
| `cmd/vswitch-ctl` | CLI:start/serve/stop、attach/detach、reserve/provision、open-port、status/stats/show、dhcp |
| `cmd/tapfd-get` | 独立 tapfd provider helper:开 tap,经 `TAPFD_SOCKET` 以 SCM_RIGHTS 递交 vnet_hdr 队列 fd |
...
```bash
make build                      # bin/<arch>/{vswitch-ctl,tapfd-get};纯 Go,CGO_ENABLED=0
make build TARGET_ARCH=aarch64  # 交叉编译(别名 amd64 / arm64)
make release                    # build/dist/sandbox-vswitch-<ver>-linux-<arch>.tar.gz
...
TAPFD_SOCKET=/tmp/recv.sock vswitch-ctl open-port sw1 --port=1   # tap fd → VMM
