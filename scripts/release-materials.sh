...
release_materials_go_payload_allowed() {
  case "$1:$2" in
    connector:bin/connector-ctl ) return 0 ;;
...
release_materials_finish() {
  ...
  while IFS= read -r toolchain; do
    ...
    if [ -n "${RELEASE_MATERIALS_GO_ENV:-}" ]; then
      ...
      toolchain_root="$(GOWORK=off command go -C "${ROOT:-$PWD}" env GOROOT)"
      installed_toolchain="$(GOWORK=off command go -C "${ROOT:-$PWD}" env GOVERSION)"
    fi
