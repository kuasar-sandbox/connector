...
if PATH="$TMP/bin:$PATH" GITHUB_REPOSITORY=kuasar-sandbox/connector   FAKE_TAG="$TAG" FAKE_TARGET=2222222222222222222222222222222222222222   FAKE_DELETE_LOG="$DELETE_LOG"   bash "$SCRIPT_DIR/delete-preview.sh" connector "$TAG" "$SOURCE_SHA" incomplete   >/dev/null 2>&1; then
  echo "test-delete-preview: deleted a Release without source ownership" >&2
  exit 1
fi
[ ! -e "$DELETE_LOG" ]

PATH="$TMP/bin:$PATH" GITHUB_REPOSITORY=kuasar-sandbox/connector   FAKE_TAG="$TAG" FAKE_TARGET="$SOURCE_SHA" FAKE_DELETE_LOG="$DELETE_LOG"   bash "$SCRIPT_DIR/delete-preview.sh" connector "$TAG" "$SOURCE_SHA" incomplete   >/dev/null
...
for arches in x86_64 'x86_64 aarch64'; do
  ...
  if PATH="$TMP/bin:$PATH" GITHUB_REPOSITORY=kuasar-sandbox/connector \
    FAKE_TAG="$TAG" FAKE_TARGET="$SOURCE_SHA" FAKE_COMPLETE_ASSETS="$assets" FAKE_DELETE_LOG="$log" \
    bash "$SCRIPT_DIR/delete-preview.sh" connector "$TAG" "$SOURCE_SHA" incomplete > "$TMP/refusal" 2>&1; then
